...
Original file is located at
    https://colab.research.google.com/drive/1q3Ds5G5Pb4OlRRgTleR4yl9Jf5Y6Wpk8
"""
...
def sentence_bump(data):
    regex=''
    try: 
        data['sentence']=data['sentence'].apply(lambda x: ' '.join(re.findall('<e1>*|<\/e1>*|<e2>*|<\/e2>*|[a-zA-Z]*', x)))
        ...
        data=[re.sub('cannot', 'can not', x) for x in data]
...
def get_feature_sp_based(data):
    data2=sentence_bump(data)

    sentence_ft, e1_distance_ft, e2_distance_ft, dependency_direct_ft, dependency_type_ft=[], [], [], [], [] #feature
    
    for sentence_data in data2:
        edges=dependency_parsing_edge(sentence_data['sentence'])
        shortest_path=path_between_2entity(sentence_data, edges)

        e1_distance, e2_distance=create_relative_distance(sentence_data)

        #rut gon lai 
        sentence_sp=''
        e1_distance_sp, e2_distance_sp=[], []
        dependency_direct, dependency_type=[], []
        ...
        for k in shortest_path:
            ...
            if k<0:
                neg=True
            elif k>0 and neg==True:
                neg_pot=True
                break

        for i, k in enumerate(shortest_path):
            if k=='N':
                continue
            try:
                sentence_sp+=sentence_data['token'][i]+' '
            except IndexError:
                print(sentence_data)
                print(shortest_path)
                raise IndexError
            e1_distance_sp.append(e1_distance[i])
            e2_distance_sp.append(e2_distance[i])
            if k==0 and neg_pot==True:
                pass
            else:
                if k!=0:
                    dependency_direct.append(k/abs(k))
                else:
                    dependency_direct.append(1)
                dependency_type.append(edges[i][2]) # 2 is index of grammar type in edge list
        
        sentence_ft.append(sentence_sp.strip())
        e1_distance_ft.append(e1_distance_sp)
        e2_distance_ft.append(e2_distance_sp)
        dependency_direct_ft.append(dependency_direct)
        dependency_type_ft.append(dependency_type)
        
    
    return sentence_ft, e1_distance_ft, e2_distance_ft, dependency_direct_ft, dependency_type_ft

class RE_DataEncoder_sp_based():
    def __init__(self, vocab_size, max_len, sentences_train, dp_type_train, label_train):
        #for the sentences
        ...
        #for the grammar
        dp_type=[]
        for t in dp_type_train:
            dp_type.extend(t)
        dp_type=list(set(dp_type))
        self.dp_type2idx={v:i+1 for i, v in enumerate(dp_type)}
    ...
    def encode(self, sentence, e1_distance, e2_distance, dependency_direct, dependency_type):
        #sentence
        sequences = self.tokenizer.texts_to_sequences(sentence)
        sentences_encode = pad_sequences(sequences, maxlen=self.max_len, padding='post')

        #relative distance
        e1_distance=pad_sequences(e1_distance, maxlen=self.max_len, value=0, padding='post')
        e2_distance=pad_sequences(e2_distance, maxlen=self.max_len, value=0, padding='post')
        e1_distance-=e1_distance.flatten().min()
        e2_distance-=e2_distance.flatten().min()
        
        dependency_direct=pad_sequences(dependency_direct, maxlen=self.max_len, value=0, padding='post')
        dependency_direct+=2

        #grammar relantion
        for i in range(len(dependency_type)):
            for j in range(len(dependency_type[i])):
                dependency_type[i][j]=self.dp_type2idx[dependency_type[i][j]]
        dependency_type=pad_sequences(dependency_type, maxlen=self.max_len, value=0, padding='post')
        ...
        return sentences_encode, e1_distance, e2_distance, dependency_direct, dependency_type
...
# data=pd.read_csv('/content/drive/MyDrive/nlp/relation_extraction_new/data/kbp37-master.csv')
# data=data.dropna()
...
# train.to_csv('/content/drive/MyDrive/nlp/relation_extraction_new/data/train.csv')
# test.to_csv('/content/drive/MyDrive/nlp/relation_extraction_new/data/test.csv')

# nlp = spacy.load("en_core_web_sm")
# data = pd.read_csv('/content/drive/MyDrive/nlp/relation_extraction_new/data/'+'train'+'.csv')
# data=data[:10]
# sentences, e1_distance, e2_distance, grammar, shortest_path, label=get_feature(data)

# with open('/content/drive/MyDrive/nlp/relation_extraction_new/data/'+'train'+'_features.json', 'r') as r:
#                 data=json.load(r)
...
if __name__ == "__main__":
    ...
    for type in ['train', 'test']:
        if read_new_data==True:
            data = pd.read_csv('/content/drive/MyDrive/nlp/relation_extraction_new/data/'+type+'.csv')
            sentences, e1_distance, e2_distance, dependency_direct, dependency_type=get_feature_sp_based(data)
            label=list(data['relationship'])
            with open('/content/drive/MyDrive/nlp/relation_extraction_new/data/'+type+'_features.json', 'w') as w:
                json.dump({'sentences': sentences, 
                        'e1_distance': e1_distance, 
                        'e2_distance': e2_distance, 
                        'dependency_direct': dependency_direct,
                        'dependency_type': dependency_type,
                        'label': label
                        }, w)     
        else:
            with open('/content/drive/MyDrive/nlp/relation_extraction_new/data/'+type+'_features.json', 'r') as r:
                data=json.load(r)
            sentences=data['sentences']
            e1_distance=data['e1_distance']
            e2_distance=data['e2_distance']
        ...
        if type=='train':
            Encoder = RE_DataEncoder_sp_based(vocab_size, max_len, sentences, dependency_type, label)

        sentences_np, e1_distance_np, e2_distance_np, dependency_direct, dependency_type = Encoder.encode(sentences, 
                                                                                                              e1_distance, 
                                                                                                              e2_distance, 
                                                                                                              dependency_direct, 
                                                                                                              dependency_type,
        ...
        all_features=np.array([sentences_np, e1_distance_np, e2_distance_np, dependency_direct, dependency_type])
        np.save('/content/drive/MyDrive/nlp/relation_extraction_new/data/X_'+type+'.npy', all_features)
        np.save('/content/drive/MyDrive/nlp/relation_extraction_new/data/y_'+type+'.npy', label_np)

        #save encoder class
        with open('/content/drive/MyDrive/nlp/relation_extraction_new/data/data_encoder2.obj', 'wb') as f:
